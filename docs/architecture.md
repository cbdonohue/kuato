...
`POST /api/drafts/:draftId/ai?username=` is a no-op 503 until `OPENAI_API_KEY` or `ANTHROPIC_API_KEY` is set. The body is `{ action, question?, playerId?, playerIds? }` with actions `ask`, `scout`, `compare`, `review`, `briefing`, and `board`. Prompts are built in `src/lib/ai.ts` from the same live snapshot; `src/lib/llm.ts` calls OpenAI first when both keys exist.
